...
DadaVController.prototype = {
	init: function (param) {
		...
		_t.bgNumber = 18,_t.heightDef = 6;
		_t.unitWidth = parseFloat((_t.contentWidth / _t.bgNumber).toFixed(2)), _t.unitHeight = Math.round(((_t.unitWidth - 9) * (130 / 177) + 9));
		if (param.isEdit) {
			container.css('height', 36 * _t.unitHeight+2);
		}else{
			container.css('height', _t.heightDef * _t.unitHeight+2);
		}
		...
		if (param.isEdit) {
			var svg = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='2000' vector-effect='non-scaling-stroke' height='" + _t.unitHeight + "'>";
			for (var i = 0; i < 20; i++) {
				svg += "<rect stroke='#D7DBDE' stroke-width='1' vector-effect='non-scaling-stroke' fill='none' x='" + (4.5 + _t.unitWidth * i) + "' y='4.5' width='" + (_t.unitWidth - 9) + "' height='" + (_t.unitHeight - 9) + "'/>";
			}
			svg += "</svg>";
			var rpsvg = svg.replace(/\</g,'%3C').replace(/\>/g,'%3E').replace(/#/g,'%23');
			container.css({'background-image': "url(\"" + rpsvg + "\")"});
			_t.initChartAction();
		}
	...
	initData: function (dataList) {
		...
		var data=dataList[0]||{};
		if((data.top_num+data.h_num)>_t.heightDef){
			var exterNmu = _t.param .isEdit?6:1;
			$(_t.containerId).css('height', (data.top_num+data.h_num+exterNmu) * _t.unitHeight);
		}
	...
	getChartUnitHtml: function (unitId, chartId, src) {
		...
		// str += '</div> ';
		if(_t.param.isEdit){
			str += '<iframe frameborder="0" class="con" src="' + src + '#/2" id="iframe_'+unitId+'"></iframe>';
		}else{
			str += '<iframe frameborder="0" class="con" src="' + src + '#/1" id="iframe_'+unitId+'"></iframe>';
		}
